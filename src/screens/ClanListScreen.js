...
import { createTables, getClans, addClan, deleteClan } from "../database/db";
...
export default function ClanListScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      Alert.alert("SQLite", e?.message || "No se pudo leer clanes");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await createTables(); // asegura tablas/migraciones
      await load();
    })();
  ...
  const askDeleteClan = (item) => {
    Alert.alert(
      "Eliminar clan",
      `¿Eliminar "${item.name}"?\nSe borrarán también sus miembros.`,
      [
